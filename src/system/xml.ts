...
export type Attributes = Maybe<Record<string, string | number>>;
...
export interface TextElement<T extends string = string> {
  type: "text";
  text: T;
}
...
export function makeNodeElement
  <N extends string, A extends Attributes, C extends Children>
  (name: N, attributes: A, children: C = [][0]): NodeElement<N, A, C> {
    return { type: "element", name, attributes, children };
  }

export function makeTextElementSchema<T extends string>(text: z.ZodType<T>) {
  return z.strictObject({
    type: z.literal("text"),
    text
  });
