...
export class ExcelStore extends FileStore<Rebate[], ExcelMeta> {
  public getFileFromItem(item: ExcelMeta): Reply<string> {
    return good(path.join(this.directory, item.quarter.toString(), item.name));
  }

  public getItemFromFile(file_path: string): Reply<ExcelMeta> {
  ...
  serialize(data: Rebate[]): Reply<Buffer> {
    ...
    return good(XLSX.write(book, { type: "buffer" }));
  }
  
  deserialize(): Reply<Rebate[]> {
    return bad("Cannot deserialize Excel files.");
  }
}